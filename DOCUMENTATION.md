...
- Response: inserted document IDs + per-document failures
...
- Response: `rfp_id` and parsed question IDs
...
- Content-Type: `application/json`
- Body: `{ "rfp_id": <int> }`
- Response: structured RFP -> questions -> answers payload
...
1. Set env vars (`LLM_API_KEY`).
2. Start services (`bin/launch_server.sh`).
4. Use UI (http://127.0.0.1:5173/) with sample documents in `/docs` to run:
   - upload company docs,
   - upload RFP,
   - bulk-generate answers.
5. (Optional): run pytest to check output of tests
